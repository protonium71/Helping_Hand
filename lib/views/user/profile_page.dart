...
class _ProfilePageState extends State<ProfilePage> {
  ...
  _get_volunteredTime(List<dynamic> history) async {
    ...
    for (int i = 0; i < history.length; i++) {
      final e1 = await FirebaseFirestore.instance
          .collection('events')
          .where('eventid', isEqualTo: history[i])
          .get();
      ...
      totalHours += eDate.hour - sDate.hour;
      totalMin += eDate.minute - sDate.minute;
    }

    totalHours += (totalMin + 59) ~/ 60;
    if (mounted) {
      setState(() {
        volunteeringHours = totalHours;
      });
    }
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => const SignedEventsPage()));
                  },
                  child: Container(
                    width: width * 0.9,
                    height: height * 0.07,
